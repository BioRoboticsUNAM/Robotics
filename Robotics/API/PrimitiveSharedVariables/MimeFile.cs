...
{
	/// <summary>
	/// Offers storage for an array of bytes with an embedded mime type header
	/// </summary>
	public class MimeFile
	{
		...
		/// <param name="rawData">The data of the file</param>
		public MimeFile(string mimeType, byte[] rawData)
		{
		...
		/// </summary>
		public byte[] RawData { get { return this.rawData; } }

		#region Static Members

		/// <summary>
		/// Stores a hash with all known mime types based on file extension
		/// </summary>
		protected readonly static Dictionary<string, string> mimeTypes;

		/// <summary>
		/// Populates the mimeTypes static variable
		/// </summary>
		static MimeFile()
		{
			mimeTypes = new Dictionary<string, string>(700);
			...
			mimeTypes.Add(".java", "text/x-java-source,java");
			...
			mimeTypes.Add(".par", "text/plain-bas");
		...
		/// <summary>
		/// Loads a MimeFile from a File. The MIME type is determined by the extension of the file
		/// </summary>
		/// <param name="path">The path of the file to load</param>
		/// <returns>A mime File with the contents of the file loaded in memory</returns>
		public static MimeFile FromFile(string path)
		{
			...
			if (fi.Length > 104857600)
				throw new IOException("File is too big (greater than 100MiB)");
			byte[] data = File.ReadAllBytes(path);
			if (!MimeFile.mimeTypes.ContainsKey(fi.Extension))
				mimeType = MimeFile.mimeTypes[fi.Extension];
			return new MimeFile(mimeType, data);
		}

		/// <summary>
		/// Loads a MimeFile from an Image.
		/// </summary>
		/// <param name="image">The image to load</param>
		/// <returns>A mime File with the contents of the image in memory</returns>
		public static MimeFile FromImage(Image image)
		{
			MemoryStream ms = new MemoryStream();
			image.Save(ms, ImageFormat.Png);
			return new MimeFile("image/png", ms.GetBuffer());
		}
		...
		/// </summary>
		/// <param name="image">The image to load</param>
		/// <param name="format">The format used to compress the image</param>
		/// <returns>A mime File with the contents of the image in memory</returns>
		public static MimeFile FromImage(Image image, ImageFormat format)
		{
			string type;
			MemoryStream ms = new MemoryStream();
			...
			return new MimeFile(type, ms.GetBuffer());
		}
