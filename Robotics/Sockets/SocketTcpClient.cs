...
namespace System.Net.Sockets
{
	...
	public class SocketTcpClient : SocketTcpServerClientBase
	{
		...
		/// </summary>
		public SocketTcpClient() : this (2000)
		{
		...
		public SocketTcpClient(int port):base(port)
		{
		...
		/// <param name="port">The connection port for the socket</param>
		public SocketTcpClient(IPAddress serverAddress, int port):this(port)
		{
	...
	public class SocketTcpClient
	{
		...
		/// </summary>
		/// <param name="port">the connection port for the socket</param>
		public SocketTcpClient(int port):this()
		{
			Port = port;
		}
		...
		/// <param name="serverAddress">The server IP Address</param>
		/// <param name="port">The connection port for the socket</param>
		public SocketTcpClient(IPAddress serverAddress, int port):this()
		{
			ServerAddress = serverAddress;
			Port = port;
		}
		...
		/// <summary>
		/// Represents the method that will handle the data received event of a SocketTcpClient object
		/// </summary>
		...
		private void dataReceived(IAsyncResult result)
		{
			try
			{
				if (socket.Connected && (socket.EndReceive(result) != 0))
				{
					if (DataReceived != null)
						DataReceived(socket, (byte[])rxBuffer.Clone());
					lastString = ASCIIEncoding.ASCII.GetString(rxBuffer).Trim(new char[] { '\0'});
					rxBuffer.Initialize();
					socket.BeginReceive(
						rxBuffer,
						0,
