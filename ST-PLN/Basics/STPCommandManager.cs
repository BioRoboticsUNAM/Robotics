...
namespace SimpleTaskPlanner
{
	...
	public class STPCommandManager : CommandManager
	{
		...
		private void LoadJustinaCommands()
		{
			...
			this.justinaCmdAndResp[(int)JustinaCommands.OBJ_FND_findOnTable] = new JustinaCmdAndResp("oft_findontable");
		...
		public void ARMS_ra_relpos(double x, double y, double z, double roll, double pitch, double yaw, double elbow, int timeOut_ms)
		{
			string parameters = x.ToString("0.00") + " " + y.ToString("0.00") + " " + z.ToString("0.00") + " " +
								 roll.ToString("0.00") + " " + pitch.ToString("0.00") + " " + yaw.ToString("0.00") + " " +
								elbow.ToString("0.00");
			...
			this.SetupAndSendCommand(JustinaCommands.ARMS_ra_relpos, parameters);
			this.WaitForResponse(JustinaCommands.ARMS_ra_relpos, timeOut_ms);
		}
		...
		public bool HEAD_getAngles(int timeOut_ms, out double[] angles)
		{
			...
			if (!this.WaitForResponse(JustinaCommands.HEAD_lookat, timeOut_ms))
				return false;
